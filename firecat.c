...
void shovel_data_by_index(const int index);
...
struct socket_pair_t pipes[2000] = {0};
...
int put_tunnel(const int t)
{
	int i;
	for (i = 0; i < sizeof(pipes) / sizeof(pipes[0]); ++i)
	{
		if (pipes[i].tunnel == -1 && pipes[i].client != -1)
		{
			pipes[i].tunnel = t;
			return i;
		}
...
int put_client(const int c)
{
	int i;
	for (i = 0; i < sizeof(pipes) / sizeof(pipes[0]); ++i)
	{
		if (pipes[i].client == -1 && pipes[i].tunnel != -1)
		{
			pipes[i].client = c;
			return i;
		}
...
void shovel_data_by_index(const int index)
{
	...
	if (0 == i)
	{
		if (send(pipes[index].tunnel, "OK", 2, 0) == -1)
		{
			perror("ERROR: send()");
			exit(1);
		}
		i = shovel_data(pipes[index].tunnel, pipes[index].client);
		close(pipes[index].tunnel);
		close(pipes[index].client);
		pipes[index].tunnel = pipes[index].client = -1;
	}
	else if (-1 == i)
	{
		printf("ERROR: fork() == %d\n", i);
...
int do_consultant(const int tunnelPort, const int servicePort)
{
	...
	{
		FD_ZERO(&acpt);
		FD_SET(tunnelSock, &acpt);
		FD_SET(serviceSock, &acpt);
		if (select(max(tunnelSock, serviceSock) + 1, &acpt, NULL, NULL, NULL) == -1)
			return 1;
		if (FD_ISSET(tunnelSock, &acpt))
		{
			i = sizeof(targetAddr);
			...
			index = put_tunnel(targetSock);
			if (index >= 0)
			{
				if (pipes[index].client != -1)
				{
					shovel_data_by_index(index);
				}
			}
		}
		if (FD_ISSET(serviceSock, &acpt))
		{
			i = sizeof(clientAddr);
			...
			index = put_client(clientSock);
			if (index >= 0)
			{
				if (pipes[index].tunnel != -1)
				{
					shovel_data_by_index(index);
				}
			}
